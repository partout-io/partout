/*
 * SPDX-FileCopyrightText: 2026 Davide De Rosa
 *
 * SPDX-License-Identifier: GPL-3.0
 */

#include "crypto/function_table.h"

#pragma clang assume_nonnull begin

bool pp_windows_crypto_init_seed(const uint8_t *src,
                                 const size_t len);

pp_crypto_ctx _Nullable pp_windows_crypto_aead_create(const char *cipher_name,
                                                      size_t tag_len,
                                                      size_t id_len,
                                                      const pp_crypto_keys *_Nullable keys);
void pp_windows_crypto_aead_free(pp_crypto_ctx ctx);

pp_crypto_ctx _Nullable pp_windows_crypto_cbc_create(const char *_Nullable cipher_name,
                                                     const char *digest_name,
                                                     const pp_crypto_keys *_Nullable keys);
void pp_windows_crypto_cbc_free(pp_crypto_ctx ctx);

pp_crypto_ctx _Nullable pp_windows_crypto_ctr_create(const char *cipher_name,
                                                     const char *digest_name,
                                                     size_t tag_len,
                                                     size_t payload_len,
                                                     const pp_crypto_keys *_Nullable keys);
void pp_windows_crypto_ctr_free(pp_crypto_ctx ctx);

#pragma clang assume_nonnull end

pp_crypto_function_table pp_crypto_function_table_native_windows(void) {
    pp_crypto_function_table table = pp_crypto_function_table_mbed();
    table.name = "native-windows";
    table.init_seed = pp_windows_crypto_init_seed;
    table.aead_create = pp_windows_crypto_aead_create;
    table.aead_free = pp_windows_crypto_aead_free;
    table.cbc_create = pp_windows_crypto_cbc_create;
    table.cbc_free = pp_windows_crypto_cbc_free;
    table.ctr_create = pp_windows_crypto_ctr_create;
    table.ctr_free = pp_windows_crypto_ctr_free;
    return table;
}
