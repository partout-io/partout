...
public final class NETunnelController: TunnelController {
    ...
    public func clearTunnelSettings(_ tunnel: IOInterface) async {
        do {
            pp_log_id(profile.id, .os, .info, "Clear tunnel settings")
            let dummy = NEPacketTunnelNetworkSettings(tunnelRemoteAddress: "127.0.0.1")
            try await provider?.setTunnelNetworkSettings(dummy)
        } catch {
    ...
    public func cancelTunnelConnection(with error: Error?) {
        ...
        if let error {
            pp_log_id(profile.id, .os, .fault, "Dispose tunnel: \(error)")
        } else {
            pp_log_id(profile.id, .os, .notice, "Dispose tunnel")
        }
...
private extension NETunnelController {
    func logReleasedProvider() {
        pp_log_id(profile.id, .os, .info, "NETunnelController: NEPacketTunnelProvider released")
    }
}
