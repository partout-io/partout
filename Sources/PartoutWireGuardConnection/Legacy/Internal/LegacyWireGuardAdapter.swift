...
private enum LegacyWireGuardAdapterState {
...
class LegacyWireGuardAdapter: @unchecked Sendable {
    ...
    /// Adapter delegate.
    private weak var delegate: LegacyWireGuardAdapterDelegate?
    ...
    /// Adapter state.
    private var state: LegacyWireGuardAdapterState = .stopped
    ...
    /// - Parameter delegate: an instance of `LegacyWireGuardAdapterDelegate`. Internally stored
    ///   as a weak reference.
    /// - Parameter backend: a backend implementation.
    /// - Parameter logHandler: a log handler closure.
    init(with delegate: LegacyWireGuardAdapterDelegate, backend: WireGuardBackend, logHandler: @escaping LogHandler) {
    ...
    func start(tunnelConfiguration: TunnelConfiguration, completionHandler: @escaping @Sendable (LegacyWireGuardAdapterError?) -> Void) {
        workQueue.async {
    ...
    /// Stop the tunnel.
    /// - Parameter completionHandler: completion handler.
    func stop(completionHandler: @escaping @Sendable (LegacyWireGuardAdapterError?) -> Void) {
        workQueue.async {
            switch self.state {
    ...
    private func setupLogHandler() {
        ...
        backend.setLogger(context: context) { context, logLevel, message in
            ...
            let unretainedSelf = Unmanaged<LegacyWireGuardAdapter>.fromOpaque(context)
                .takeUnretainedValue()
    ...
    /// - Throws: an error of type `LegacyWireGuardAdapterError`.
    ...
    private func setNetworkSettings(_ networkSettings: NEPacketTunnelNetworkSettings) throws {
        ...
        if condition.wait(until: Date().addingTimeInterval(setTunnelNetworkSettingsTimeout)) {
            if let systemError = systemError {
                throw LegacyWireGuardAdapterError.setNetworkSettings(systemError)
            }
        } else {
    ...
    private func resolvePeers(for tunnelConfiguration: TunnelConfiguration) throws -> [WireGuardEndpoint?] {
        ...
        guard resolutionErrors.isEmpty else {
            throw LegacyWireGuardAdapterError.dnsResolution(resolutionErrors)
        }
    ...
    private func startWireGuardBackend(wgConfig: String) throws -> Int32 {
        guard let tunnelFileDescriptor = self.tunnelFileDescriptor else {
            throw LegacyWireGuardAdapterError.cannotLocateTunnelFileDescriptor
        }

        let handle = backend.turnOn(settings: wgConfig, tun_fd: tunnelFileDescriptor)
        if handle < 0 {
            throw LegacyWireGuardAdapterError.startWireGuardBackend(handle)
        }
        #if os(iOS)
