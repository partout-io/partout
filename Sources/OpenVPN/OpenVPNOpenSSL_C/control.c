...
size_t ctrl_pkt_capacity_alg(const ctrl_pkt_t *pkt, const ctrl_pkt_alg *alg) {
    ...
    const size_t enc_capacity = alg->crypto->meta.encryption_capacity(alg->crypto, plain_capacity);
...
size_t ctrl_pkt_serialize_auth(uint8_t *dst,
    ...
    const size_t digest_len = alg->crypto->meta.digest_len;
    ...
    *(uint32_t *)ptr = endian_htonl(alg->timestamp);
    ...
    size_t dst_len;
    if (!alg->crypto->encrypter.encrypt(alg->crypto, dst, &dst_len, subject, subject_len, NULL, error)) {
        return 0;
    }
...
size_t ctrl_pkt_serialize_crypt(uint8_t *dst,
    ...
    *(uint32_t *)ptr = endian_htonl(alg->timestamp);
    ...
    size_t enc_msg_len;
    if (!alg->crypto->encrypter.encrypt(alg->crypto, dst + ad_len, &enc_msg_len, msg->bytes, msg->length, &flags, error)) {
        zd_free(msg);
        return 0;
    }
    zd_free(msg);
