...
@OpenVPNActor
final class LegacyOpenVPNSession {
    enum SessionState {
        case stopped
    ...
    private var negotiators: [UInt8: LegacyNegotiator]

    private var dataChannels: [UInt8: LegacyDataChannel]
...
extension LegacyOpenVPNSession: OpenVPNSessionProtocol {
...
extension LegacyOpenVPNSession {
    ...
    var currentNegotiator: LegacyNegotiator? {
    ...
    var currentDataChannel: LegacyDataChannel? {
    ...
    func newNegotiator(on link: LinkInterface) throws -> LegacyNegotiator {
        let negOptions = LegacyNegotiator.Options(
            configuration: configuration,
    ...
    func addNegotiator(_ negotiator: LegacyNegotiator) {
    ...
    func dataChannel(for key: UInt8) -> LegacyDataChannel? {
        dataChannels[key]
    }
...
private extension OpenVPNCryptoProtocol {
    func newControlChannel(
        ...
        configuration: OpenVPN.Configuration
    ) throws -> LegacyControlChannel {
        let channel: LegacyControlChannel
        if let tlsWrap = configuration.tlsWrap {
            switch tlsWrap.strategy {
            case .auth:
                channel = try LegacyControlChannel(
                    ctx,
                    prng: prng,
                    crypto: self,
                    authKey: tlsWrap.key,
            ...
            @unknown default:
                channel = LegacyControlChannel(ctx, prng: prng)
            }
        } else {
            channel = LegacyControlChannel(ctx, prng: prng)
        }
