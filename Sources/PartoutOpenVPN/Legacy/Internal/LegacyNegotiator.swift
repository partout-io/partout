...
final class LegacyNegotiator {
    struct Options {
        ...
        let onConnected: (UInt8, LegacyDataChannel, PushReply) async -> Void
    ...
    private var channel: LegacyControlChannel
    ...
    private var state: State {
        didSet {
            pp_log(ctx, .openvpn, .info, "LegacyNegotiator: \(key) -> \(state)")
        }
    }
    ...
    private var pendingPackets: [UInt32: LegacyPacket]

    private var authenticator: LegacyAuthenticator?
    ...
    private init(
        ...
        link: LinkInterface,
        channel: LegacyControlChannel, // TODO: #29, abstract this for testing
        prng: PRNGProtocol,
        tlsFactory: @escaping () -> OpenVPNTLSProtocol,
        cryptoFactory: @escaping () -> OpenVPNCryptoProtocol,
        options: Options
    ...
    func forRenegotiation(initiatedBy newRenegotiation: RenegotiationType) -> LegacyNegotiator {
        guard let history else {
            pp_log(ctx, .openvpn, .error, "LegacyNegotiator has no history (not connected yet?)")
            return self
        }
        let newKey = Constants.ControlChannel.nextKey(after: key)
        return LegacyNegotiator(
            ctx,
            key: newKey,
            history: history,
...
extension LegacyNegotiator {
    ...
    func readInboundPacket(withData packet: Data, offset: Int) throws -> LegacyPacket {
        try channel.readInboundPacket(withData: packet, offset: 0)
    }

    func enqueueInboundPacket(packet controlPacket: LegacyPacket) -> [LegacyPacket] {
        channel.enqueueInboundPacket(packet: controlPacket)
    }

    func handleControlPacket(_ packet: LegacyPacket) throws {
    ...
    func sendAck(for controlPacket: LegacyPacket, to link: LinkInterface) {
        Task {
...
private extension LegacyNegotiator {
    ...
    func enqueueControlPackets(code: LegacyPacketCode, key: UInt8, payload: Data) throws {
...
private extension LegacyNegotiator {
    func privateHandleControlPacket(_ packet: LegacyPacket) throws {
    ...
    func privateSendAck(for controlPacket: LegacyPacket, to link: LinkInterface) async throws {
        do {
    ...
    func handleControlData(_ data: LegacyZD) throws {
...
private extension LegacyNegotiator {
    ...
    func newDataChannel(with history: NegotiationHistory) throws -> LegacyDataChannel {
...
//        pp_log(ctx, .openvpn, .info, "\tremoteSessionId: \(remoteSessionId.toHex())")
        ...
        let dataPath = DataPath(
            ...
            usesReplayProtection: Constants.DataChannel.usesReplayProtection
        )
        return LegacyDataChannel(ctx, key: key, dataPath: dataPath)
    }
}
