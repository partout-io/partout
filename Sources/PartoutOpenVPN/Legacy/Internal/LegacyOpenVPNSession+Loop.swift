...
extension LegacyOpenVPNSession {
    func loopTunnel() {
        runInActor { [weak self] in
            guard let self else {
                pp_log(.global, .openvpn, .debug, "Ignore TUN read from outdated LegacyOpenVPNSession")
                return
            }
    ...
    func loopLink() {
        link?.setReadHandler { [weak self] packets, error in
            self?.runInActor { [weak self] in
                guard let self else {
                    pp_log(.global, .openvpn, .debug, "Ignore LINK read from outdated LegacyOpenVPNSession")
                    return
                }
                if let error {
...
private extension LegacyOpenVPNSession {
    func receiveLink(packets: [Data]) throws {
        ...
        for packet in packets {
            ...
            let controlPacket: LegacyPacket
            do {
