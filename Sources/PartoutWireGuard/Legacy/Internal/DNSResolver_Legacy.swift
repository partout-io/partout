...
extension DNSResolver {
    ...
    static func resolveSync(endpoints: [WireGuardEndpoint?]) -> [Result<WireGuardEndpoint, DNSResolutionError>?] {
        ...
        return endpoints.concurrentMap(queue: resolverQueue) { endpoint -> Result<WireGuardEndpoint, DNSResolutionError>? in
    ...
    private static func resolveSync(endpoint: WireGuardEndpoint) throws -> WireGuardEndpoint {
        ...
        if let ipv4Address = ipv4Address {
            return WireGuardEndpoint(host: .ipv4(ipv4Address), port: endpoint.port)
        } else if let ipv6Address = ipv6Address {
            return WireGuardEndpoint(host: .ipv6(ipv6Address), port: endpoint.port)
        } else {
            // Must never happen
            fatalError()
        }
...
extension WireGuardEndpoint {
    func withReresolvedIP() throws -> WireGuardEndpoint {
        #if os(iOS) || os(tvOS)
        let hostname: String
        switch host {
