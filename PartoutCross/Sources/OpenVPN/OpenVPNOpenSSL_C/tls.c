...
int tls_channel_verify_peer(int ok, X509_STORE_CTX *_Nonnull ctx) {
    if (!ok) {
        ...
        tls_channel_t *tls = SSL_get_ex_data(ssl, tls_channel_ex_data_idx);
...
tls_channel_t *tls_channel_create(const tls_channel_options *opt, tls_error_code *error) {
    ...
    tls_channel_t *tls = pp_alloc_crypto(sizeof(tls_channel_t));
    tls->opt = opt;
...
void tls_channel_free(tls_channel_t *tls) {
    if (!tls) return;
...
char *tls_channel_ca_md5(const tls_channel_t *tls) {
