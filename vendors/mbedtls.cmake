...
if(PP_USE_SYSTEM_VENDORS)
    ...
    if(MbedTLS_FOUND AND TARGET MbedTLS::mbedtls AND TARGET MbedTLS::mbedx509 AND TARGET MbedTLS::mbedcrypto)
        ...
        get_target_property(PARTOUT_MBEDTLS_INCLUDE_DIRS MbedTLS::mbedtls INTERFACE_INCLUDE_DIRECTORIES)
        list(GET PARTOUT_MBEDTLS_INCLUDE_DIRS 0 PARTOUT_MBEDTLS_INCLUDE_DIR)
        get_target_property(PARTOUT_MBEDTLS_LIBRARY MbedTLS::mbedtls IMPORTED_LOCATION)
        if(NOT PARTOUT_MBEDTLS_LIBRARY)
            get_target_property(PARTOUT_MBEDTLS_CONFIGS MbedTLS::mbedtls IMPORTED_CONFIGURATIONS)
            if(PARTOUT_MBEDTLS_CONFIGS)
                list(GET PARTOUT_MBEDTLS_CONFIGS 0 PARTOUT_MBEDTLS_CONFIG)
                string(TOUPPER "${PARTOUT_MBEDTLS_CONFIG}" PARTOUT_MBEDTLS_CONFIG)
                get_target_property(PARTOUT_MBEDTLS_LIBRARY MbedTLS::mbedtls "IMPORTED_LOCATION_${PARTOUT_MBEDTLS_CONFIG}")
            endif()
        endif()
        if(NOT PARTOUT_MBEDTLS_LIBRARY)
            message(FATAL_ERROR "Unable to locate the system MbedTLS library")
        endif()
        get_filename_component(PARTOUT_MBEDTLS_LIBRARY_DIR "${PARTOUT_MBEDTLS_LIBRARY}" DIRECTORY)
    ...
    if(PkgConfig_FOUND)
        ...
        if(MBEDTLS_PKG_FOUND AND TARGET PkgConfig::MBEDTLS_PKG)
            set(PP_MBEDTLS_IS_SYSTEM ON)
            list(GET MBEDTLS_PKG_INCLUDE_DIRS 0 PARTOUT_MBEDTLS_INCLUDE_DIR)
            list(GET MBEDTLS_PKG_LIBRARY_DIRS 0 PARTOUT_MBEDTLS_LIBRARY_DIR)
            add_library(MbedTLSInterface INTERFACE)
